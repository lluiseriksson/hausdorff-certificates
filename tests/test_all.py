...
def test_certificate_invalid_matrix_kind_reports_failure():
    b = moments_lebesgue(12)
    cert = certify_exact("hilbert", b, "hankel_H", 5)
    obj = json.loads(cert.to_json())
    obj["matrix"]["kind"] = "not_a_matrix_kind"
...
def test_certificate_missing_matrix_kind_reports_failure():
    b = moments_lebesgue(12)
    cert = certify_exact("hilbert", b, "hankel_H", 5)
    obj = json.loads(cert.to_json())
    del obj["matrix"]["kind"]
...
def test_certificate_missing_matrix_n_reports_failure():
    b = moments_lebesgue(12)
    cert = certify_exact("hilbert", b, "hankel_H", 5)
    obj = json.loads(cert.to_json())
    del obj["matrix"]["N"]
...
def test_certificate_boolean_matrix_n_reports_failure():
    b = moments_lebesgue(12)
    cert = certify_exact("hilbert", b, "hankel_H", 5)
    obj = json.loads(cert.to_json())
    obj["matrix"]["N"] = True
    ...
    ok, msg = verify_obj(obj)
    assert not ok
    assert msg == "matrix rebuild failed: matrix N must be a nonnegative integer, got True"
...
def test_certificate_short_witness_reports_dimension_failure():
    b = moments_from_atoms([(F(1, 2), F(1, 3)), (F(1, 2), F(2, 3))], 12)
    cert = certify_exact("bad_theta", b, "hankel_L_theta", 5, theta=F(1, 2))
    obj = json.loads(cert.to_json())
    obj["certificate"]["v"] = obj["certificate"]["v"][:-1]

    ok, msg = verify_obj(obj)
    assert not ok
    assert msg == "witness vector length 5 does not match matrix size 6"


def test_certificate_short_interval_witness_reports_dimension_failure():
...
def test_manifest_cli_rejects_path_entries(tmp_path):
    manifest = tmp_path / "manifest.json"
    manifest.write_text(
        json.dumps(
            {
                "format": "hausdorff-certificates-manifest/1",
                "files": {"../outside.cert.json": "0" * 64},
            },
            sort_keys=True,
            indent=1,
        )
